...
void SPI_Receive(SPI_TypeDef* SPIx, uint8_t* rxData, uint16_t size) {
    for (uint16_t i = 0; i < size; i++) {
        while (!(SPIx->SR & SPI_SR_TXE));  // TX empty
        *((__IO uint8_t*)&SPIx->DR) = 0xFF; // Dummy byte to generate clock
...
void nrf_TX_payload(NRF24L01* nrf, uint8_t* data){
    uint8_t spiTXbuf[33];
    spiTXbuf[0] = 0xA0;  // W_TX_PAYLOAD 명령어

    // 데이터 복사
    for (int i = 0; i < nrf->para.length; i++) {
        spiTXbuf[i + 1] = data[i];
    }
    ...
    // CSN Low → SPI 전송 → CSN High
    LL_GPIO_ResetOutputPin(nrf->chip_select_port,nrf->chip_select_pin);
    SPI_Transmit(nrf->SPI, spiTXbuf, nrf->para.length + 1);
    LL_GPIO_SetOutputPin(nrf->chip_select_port,nrf->chip_select_pin);
}
